...
class HistoryEmbedding(nn.Module):
    ...
    @torch.no_grad()
    def user_emb(self):
        interaction_vec = self.interactions["user"][:-1, :-1]

        num_hist = interaction_vec.sum(dim=1, keepdim=True)
        num_hist = torch.clamp(num_hist, min=1.0)
        projection_vec = self.projection["user"](interaction_vec) / torch.sqrt(num_hist)
    ...
    @torch.no_grad()
    def item_emb(self):
        interaction_vec = self.interactions["item"][:-1, :-1]

        num_hist = interaction_vec.sum(dim=-1, keepdim=True)
        num_hist = torch.clamp(num_hist, min=1.0)
        projection_vec = self.projection["item"](interaction_vec) / torch.sqrt(num_hist)
